...
ROOT = ""
#from mg_testing import VCycleCG as VCycle
...
def _main():
    ...
    traj = load(ROOT+'traj.npy')[:Nt+1] if MASTER else None
    shp = ( (traj.shape[0]-1), traj.shape[1] ) if MASTER else None

    ns = IF3DParallel(k, nu, dt, traj, shp, top=True)

    vcycle = VCycle(ns, levels)
    ...
    if MASTER:
        print 'init res: {nr:.6e}, rel_res: {rr:.3e}'.format(nr=n_res, rr=n_res/n_rhs) 
    ...
    for i in xrange(100):
        ...
        if MASTER:
            print 'i:{}, res: {:.6g}, rel_res: {:.3e}'.format(i, n_res, rel_res)
            print '- REC:', rec[-1]
            
            save(ROOT +'MG_REC.npy', array(rec) )
    ...
    if MASTER:
        ...
        save(ROOT+'MG_results.npy', w_full)
